...
def appearance(intervals: dict[str, list[int]]) -> int:
    ...
    lists are ordered by ascending of elements with even (odd) indexes.
    """
    ...
    total_intersection: list[int] = (
            intersected_intervals(
                    intersected_intervals(lesson, union_intervals(tutor)), union_intervals(pupil))
    )
...
def union_intervals(lst: list[int]) -> list[int]:
    """Creates new list of union of overlapping intervals from the origin list."""
    result: list[int] = []
    result.extend([lst[0], lst[1]])
    i = 0
    for j in range(1, len(lst) // 2):
        if result[2 * i + 1] > lst[2 * j]:
            result[2 * i + 1] = max(result[2 * i + 1], lst[2 * j + 1])
        else:
            i += 1
            result.extend([lst[2 * j], lst[2 * j + 1]])
    return result
